...
const ModelValidatorTaskS_1 = require("./ModelValidatorTaskS");
const __1 = require("..");
/**
...
class ModelValidatorSys {
    constructor(errorSys) {
        ...
        this.vValidatorTask = new ModelValidatorTaskS_1.ModelValidatorTaskS(this);
    }
    // ================================================================
    /**
    ...
    fValid(aRules, data) {
        ...
        for (let i = 0; i < akRules.length; i++) {
            ...
            if (bExist && bDpend && v.type == __1.ModelRulesT.str) {
                this.fTypeStr(k, v);
            }
            // Обработка [boolean] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.boolean) {
                this.fTypeBool(k, v);
            }
            // Обработка [integer] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.int) {
                this.fTypeInt(k, v);
            }
            // Обработка [enum] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.enum) {
                this.fTypeEnum(k, v);
            }
            // Обработка [text] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.text) {
                this.fTypeText(k, v);
            }
            // Обработка [json] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.json) {
                this.fTypeJson(k, v);
            }
            // Обработка [decimal] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.decimal) {
                this.fTypeDecimal(k, v);
            }
            // Обработка [arrayNumbers] значений
            if (bExist && bDpend && v.type === __1.ModelRulesT.arrayNumbers) {
                this.fTypeArrayNumbers(k, v);
            }
            // Обработка [object] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.object) {
                this.fTypeObject(k, v);
            }
            // Обработка [array] значений
            if (bExist && bDpend && v.type == __1.ModelRulesT.array) {
                this.fTypeArray(k, v);
            }
            // =================================================
            // Логические проверки
     ...
     * @param vRule
     */
    fTypeEnum(kRule, vRule) {
        let bOk = true;
        if (!this.vValidatorTask.fValidEnum(kRule, vRule.if)) {
            this.okResult = false;
            this.abValidOK[kRule] = false;
    ...
    fMore(kRule, vRule) {
        let bOk = true;
        if (vRule.type == __1.ModelRulesT.int || vRule.type == __1.ModelRulesT.decimal) {
    ...
    fMaxLen(kRule, vRule) {
        ...
        if (vRule.type == __1.ModelRulesT.text || vRule.type == __1.ModelRulesT.str) {
    ...
    fMinLen(kRule, vRule) {
        ...
        if (vRule.type == __1.ModelRulesT.text || vRule.type == __1.ModelRulesT.str) {
            if (this.vValidatorTask.fValidMinLen(kRule, vRule.min_len)) {
                ...
                this.okResult = false;
                this.errorSys.error(`valid_'${kRule}_min_len`, `${vRule.error} Количество символов меньше минимального значения = ${this.data[kRule]}`);
            }
        }
        else {
